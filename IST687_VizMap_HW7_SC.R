...
# Get New york coordinates from geocode
nyMap <- get_map(location='New York',zoom=6)

map.ny<-ggmap(nyMap)
map.ny

View(mergedDf)
# Plot All the zip codes as per median income around NY
map.nyzip <- map.ny + geom_point(data = mergedDf, aes(x=mergedDf$longitude, y=mergedDf$latitude,colour = median, factor = (mergedDf$state == "NY")),na.rm=TRUE)
map.nyzip <- map.nyzip + ggtitle("Zip codes as per median income around NY")
map.nyzip

# Density for all Zip codes in around NY
map.allnyzip <- map.ny + stat_density2d(aes(x = mergedDf$longitude, y = mergedDf$latitude, factor = (mergedDf$state == "NY")), data = mergedDf, geom = "polygon",na.rm=TRUE) +
                     scale_fill_gradient(low="black",high="green")+
                     scale_alpha(range=c(0.00,0.25))+
                     ggtitle("Plot Density for all Zip codes in around New York")+
                     theme (plot.title=element_text(lineheight=3.5,face="bold"))
map.allnyzip
